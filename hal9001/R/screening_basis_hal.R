...
screen_basis = function(basis_list,X,y, index_to_keep = NULL, return_index = F, lower.limits = -Inf, upper.limits = Inf, screen_at_which_lambda = NULL, family = "gaussian" ){
  ...
  if(!is.null(screen_at_which_lambda)){
    ...
    fit = glmnet::cv.glmnet(x = x_basis, y = y, family = family, lower.limits = lower.limits, upper.limits= upper.limits)
    ...
    fit  = glmnet::glmnet(x = x_basis, y = y, family = family, lower.limits = lower.limits, upper.limits = upper.limits)
  ...
  if(is.null(screen_at_which_lambda)){
    betas = fit$beta
    coefs = apply(as.matrix(betas),2,function(v){as.vector(which(v!=0))})
...
merge_basis = function(reduced_basis_list1,reduced_basis_list2,X){
  ...
  if(is.null(reduced_basis_list1)==0 | is.null(reduced_basis_list2)){
    return(NULL)
  }
  X=as.matrix(X)
...
get_higher_basis_up_to_three = function(reduced_basis_list,max_dim, X,y, screen_each_level){
  ...
  get_valid_two_way_ind =apply(mat, 2, function(v){any(map[[v[1]]] %in% map[[v[2]]])})
  ...
  keeper=unlist(sapply(res,function(v){nrow(v)!=0}))
  res = res[keeper]
