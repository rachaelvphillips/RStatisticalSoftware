...
#'  input columns.
basis_list_cols <- function(cols, x, orderMap) {
...
basis_of_degree  <- function(x, degree, orderMap) {
  ...
  all_basis_lists <- apply(all_cols, 2, basis_list_cols_order, x=x, orderMap = orderMap)
...
enumerate_basis <- function(x, max_degree = NULL, orderMap = rep(0, ncol(x))){
  ...
  all_bases <- lapply(degrees, function(degree) basis_of_degree_order(x, degree, orderMap))
