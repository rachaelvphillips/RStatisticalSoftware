...
OneStep2 <- R6::R6Class("OneStep2",
                    ...
                    public = list(
                      ...
                      #Computes sample vector of influence curves of target parameter at time t_tgt
                      getColMeansOld = function(A_cf, A="obs"){
                      ...
                      #This is slower than without parallelization, even with big matrices.
                      computeCleverMatLongPar = function(A_cf, A = "obs"){
                        pieces = split(private$targetPoints, seq_along(private$targetPoints) %%4)
                        print(pieces)
                        doPiece = function(i){
                          result = lapply(pieces[[i]], FUN = function(t){as.vector(self$getCleverMatSpeed(A_cf=1,t,A=1))})
                          return(result)
                        }
                        result = as.matrix(do.call(cbind, unlist(lapply(1:length(pieces), doPiece), recursive = F)))
                      ...
                      computeCleverMatLongSlow = function(A_cf, A="obs",addIndic = F){
                        return(as.matrix(do.call(rbind,lapply(1:private$t_max, self$getCleverMat, A_cf = A_cf, A = A,addIndic = addIndic))))
                      },
                      ...
                      #Performs One iteration of the one step targeting the user-defined custom targetParameter.
                      updateCustom = function(){
                        delta = private$delta

                        EIC_mat = as.matrix(self$getEICMat(A_cf="custom", A="obs", full=F))

                        weights = private$weightsCustom
                        ...
                        cleverMats1 = self$getCleverMatLong(A_cf="custom",A=1)

                        cleverMats0 = self$getCleverMatLong(A_cf="custom",A=0)

                        offset1 = (stats::qlogis(self$getHazard(A_cf=1, long = F)))
                        offset0 = (stats::qlogis(self$getHazard(A_cf=0, long = F)))

                        getDirection = function(delt){
                          dir = EIC_mean /abs(weights)
                          norm_dir = private$norm(dir, weights )
                          return(as.vector(delt*dir/norm_dir))
                        }

                        updateAllT0 = function(dir){
                          dir = as.vector(dir)
                          cleverAll = cleverMats0
                          offset_All = matrix(offset0, ncol=1)
                          update_All = stats::plogis(offset_All + cleverAll %*%  (dir))
                          return(matrix(update_All, nrow = private$n, ncol = private$t_max))
                        }
                        updateAllT1 = function(dir){
                          dir = as.vector(dir)
                          cleverAll = cleverMats1
                          offset_All = matrix(offset1, ncol=1)
                          update_All = stats::plogis(offset_All + cleverAll %*%  (dir))
                          return(matrix(update_All, nrow = private$n, ncol = private$t_max))
                        }
                        ...
                        new_haz_1 = updateAllT1(dir)
                        new_haz_0 = updateAllT0(dir)
                        ...
                        private$estimator$update(survival_curve$new(t = 1: private$t_max, hazard = as.matrix(new_haz_0)), 0)

                        private$estimator$update(survival_curve$new(t = 1: private$t_max, hazard = as.matrix(new_haz_1)), 1)
                      ...
                      #Performs one iteration of OneStep targetting the parameter S_1 - S_0 directly.
                      updateDiff = function(){
                        delta = private$delta
                      ...
                      getShrinkageDirection = function(A_cf){

                        clever =self$computeCleverMatLongSlow(A_cf=A_cf,A="obs", addIndic = T)
                        offset = as.vector(stats::qlogis(self$getHazard(A="obs")))
                        outcome = as.vector(self$get_dNt())
                        fit = glmnet::cv.glmnet(x=clever,y=outcome,offset=offset,alpha=0, family = "binomial", intercept=F, parallel = T, relax=T)
                        #fit = glmnet::glmnet(x=clever,y=outcome,offset=offset,alpha=0, family = "binomial", intercept=F, lambda = 0)

                        betas = stats::coef(fit, s = "lambda.min")[c(2:(ncol(clever)+1))]
                        # betas = coef(fit)[c(2:(ncol(clever)+1))]
                      ...
                      updateOnce = function(A_cf, search_step = T, byRisk = F,recalc=T, stop = T, debug = F, shrinkage = F, maxDelta = 0.005){
                        t=proc.time()
                        delta = private$delta
                        ...
                        if(shrinkage){
                          newPsiChange=1
                          new_haz = (updateAllT(self$getShrinkageDirection(A_cf)))
                      ...
                      #Checks if S_1 or S_0 is targeted.
                      hasConverged = function(A_cf){
                        return(private$converged[A_cf+1])
                      },
