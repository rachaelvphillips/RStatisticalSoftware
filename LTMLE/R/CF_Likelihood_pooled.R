...
#'
#' @export
CF_Likelihood_pooled <- R6Class(
  classname = "CF_Likelihood_pooled",
  portable = TRUE,
  class = TRUE,
  ...
  public = list(
    ...
    get_likelihood = function(tmle_task, node, fold_number = "full", drop_id = F, to_wide = F) {
      ...
      if (node %in% self$intervention_nodes) {
        likelihood_values <- super$get_likelihood(tmle_task, node, fold_number, drop_id = drop_id, to_wide = to_wide)
      } else {
        # dispatch to observed likelihood if not an intervention node
        # that way, we get updates to those nodes
        likelihood_values <- self$observed_likelihood$get_likelihood(tmle_task, node, fold_number, drop_id = drop_id, to_wide = to_wide)
      }
