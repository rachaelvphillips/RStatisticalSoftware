...
Param_base <- R6Class(
  ...
  public = list(
    initialize = function(observed_likelihood, ..., outcome_node = "Y") {
      ...
      if (inherits(observed_likelihood, "Targeted_Likelihood") | inherits(observed_likelihood, "Targeted_Likelihood_pooled")) {
        # register parameter with updater
        observed_likelihood$updater$register_param(self)
      } else if (inherits(observed_likelihood, "Likelihood") | inherits(observed_likelihood, "Likelihood_pooled")) {
