...
dat = task$data
sum(dat$t==2)
new_dat <- task$get_tmle_node("processN", force_time_value = 2)
new_dat$t = 2
set(new_dat, ,"processN", 1)
new_dat
newer_dat = task$generate_counterfactual_task(UUIDgenerate(), new_dat)$data
newer_dat
sum(table(newer_dat$id)==2)
```
...
sampler$sample_from_node(task, "W", 1)
```
