...
task$get_regression_task("processN")$data
...
```{r}
n_samples=10
emp_probs <- c(0.1, 0.2, 0.5, 0.4)
emp_probs <- emp_probs/ sum(emp_probs)

uniq_obs <-  c(.1,.2,.3,.4)
uniq_obs[base::sample(seq_along(uniq_obs), size = n_samples, replace = T, prob = uniq_obs)]
