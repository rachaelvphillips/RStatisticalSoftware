...
library(hal9001)
library(data.table)
density = function(x, z){
  dunif(x, 2, 4)
}
n=200
dat <- matrix(c(rbinom(n,5,0.5), rbinom(n,5,0.5)), ncol = 2)
data.table(dat)
basis_list <- hal9001::enumerate_basis(dat, 2)

basis_to_function = function(basis){
  f = function(y,x){
    as.numeric(all(c(y,x)>= basis$cutoffs) )
  }
  g <- function(y,x=2){
    integrand = function(x,y) {f(y,x)*density(y,x)}
    print(f(3,2))
    print(density(3,2))
    res = f(y,x) - integrate(integrand, lower = 2, upper = 4, x=x)$value
 
    return(res)
  }
  return(g)
}
func = basis_to_function(basis_list[[18]])
basis_list[[18]]
func(3)
...
lik = generate_likelihood_surv(task)
lik = lik$train(task)
tlik = Targeted_Likelihood_pooled$new(lik, updater = tmle3_Update$new(one_dimensional = T,constrain_step=T, delta_epsilon = 0.1, maxit = 10))
