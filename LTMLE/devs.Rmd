...
```{r}
time=8
fun = function(X,time, id){
  return(X+1
         )
}
data[3:9,fun(.SD, time, id), by = id, .SDcols = "id"]
...
```{r}
cf_data = task$data[, c("processN", "processA")]
cf_data$processN =1
cf_data$processA =1

cf_task = task$generate_counterfactual_task(task$uuid, new_data = cf_data, T)
cf_task$data

```

```{r}
haz <- task$get_tmle_node("processN")
haz
haz <- reshape(haz, idvar = "id", timevar = "t", direction = "wide")
...
lik$get_likelihoods(cf_task, c("processN"), to_wide = T)
```

```{r}
lapply(res$processN, as.data.table)

```


```{r}
H = data.table(res$processN$H)
D = res$processN$D
H
pN <- tlik$get_likelihood(task, "processN", to_wide = F)



eps <- D

submodel = function(initial) {
      plogis(qlogis(initial) + as.matrix(H) %*% eps)
}
observed <- task$get_regression_task("processN")

observed <- observed$get_data(, c("id", "t", "at_risk", "processN"))

loss_function = function(estimate, observed) {
      -1 * ifelse(observed == 1, log(estimate), log(1 - estimate))
}
keep = which(observed$at_risk==1)

pN
observed
newP <- pN[, lapply(.SD, submodel), .SDcols = "processN"]
setnames(newP, "processN")
newP
newP$t = pN$t
newP$id = pN$id
...
mean(loss_function(newP$processN[keep], observed$processN[keep] ))
mean(loss_function(pN$processN[keep], observed$processN[keep] ))
...
```{r}
data.table((res$processN$H))
data.table(res$processN$D)
tlik = Targeted_Likelihood_pooled$new(lik)
```
```{r}

param <- Param_survival$new(tlik, intervention_list = list(), outcome_node = "processN")

res = param$clever_covariates(task)
```

```{r}
dats <- likes[, c("t", "id", "processN")]
dats<- dats[which(dats$t!=0), ]
dats$t = dats$t
to_wide <- function(X){
  times <- X$t
  X <- X[, c("processN")]
...
dats <- dats[, reshape(.SD,  idvar = "id", timevar = "t", direction = "wide")]
grep("processN", colnames(dats), value = T)
```
