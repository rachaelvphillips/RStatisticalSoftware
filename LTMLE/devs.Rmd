...
D <- D +
  ...
  node("Ttilde", distr = "rconst", const = round(min(T,C) +1  ))
setD <- set.DAG(D)
dat <- sim(setD, n = 2e2)
...
npsem = list(define_lnode("W", c("W"), time =1), 
     define_lnode("A", c("A"), c("W"), time =1, summary_functions = make_summary_measure_baseline(c("W"))),
...
task = ltmle3_Task$new(data, npsem)
 task$get_tmle_node("processN", force_time_value = 1, format = T, include_time = T)

```

```{r}
task = ltmle3_Task$new(data, npsem)
 task$get_tmle_node("processN", force_time_value = 1, format = T)

 task$get_regression_task("processN", force_time_value = 1, drop_censored = F)$get_data()
