...
X = data.frame(X)
Y = 2*rowMeans((X[,1:10,drop=F]))^3 + rowMeans(X[,10:15,drop=F]-1)^2 + rnorm(n,0,1)

data = data.frame(cbind(X,Y))
colnames(data) = c(colnames(X), "y")
task = make_sl3_Task(data, covariates = colnames(X), outcome = "y")

lrnr <- make_learner(Lrnr_hal9001fast, max_degree =3, cv_select = T,bins=200,  cor_pval=0.15,screen_basis_main_terms=T, screen_basis_interactions=F)
