...
lambda = c(1,0.5)
f=make_learner_subsemble_hal(make_learner(Lrnr_hal9001, max_degree=1,cv_select=F, lambda = lambda, verbose=F), task,k=4, num_preds = length(lambda))
...
c = f$fit_object$learner_fits[[2]]$fit_object$learner_fits[[1]]$fit_object$hal_fits[[2]]$coefs[-1]
sum(abs(a))
sum(abs(c))
```
