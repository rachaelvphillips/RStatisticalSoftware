...
sl3_BigTask <- R6Class(
  ...
  inherit = sl3_Task,
  public = list(
    initialize = function(data, covariates, outcome = NULL, bm=NULL, bm_columns = NULL,
      ...
      } else if (inherits(data, "Shared_BigData")) {
      ...
      if (is.null(column_names)) {
        column_names <- as.list(data_names)
        names(column_names) <- column_names
      }
      if(!is.null(bm_columns)){
        private$.bm_columns = bm_columns
      }
      ...
      # process data
      if (inherits(data, "Shared_BigData")) {
        ...
        private$.shared_data <- Shared_BigData$new(data, bm, force_copy = FALSE)
      }
    ...
    next_in_chain = function(bm_columns = NULL, covariates = NULL, outcome = NULL, id = NULL,
  ...
  active = list(
    ...
    bm = function(){
      ...
      return(private$.shared_data$bm)

    },
    fbm = function(){
      return(private$.shared_data$fbm)
    },
    bm_columns = function(){
      cols = private$.bm_columns



      if(is.null(cols)){
        all_cols = private$.shared_data$bm_names
        return(all_cols)
      }
      return(cols)
...
#' @export
#
make_sl3_BigTask <- sl3_BigTask$new
