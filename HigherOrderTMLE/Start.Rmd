...
likelihood_tilde <- Likelihood$new(factor_list_tilde)
likelihood_tilde <- likelihood_tilde$train(task)
tlik <- Targeted_Likelihood$new(likelihood, updater = list(maxit = 300, convergence_type = "scaled_var", cvtmle = F, constrain_step = T, one_dimensional = T, delta_epsilon = c( 0.005)))
param <- Param_TSM_higher_order$new(tlik, likelihood_tilde)
est <- param$estimates(task)
est0 <- mean(est$psi)
