...
```{r}
write.csv(results, "rerun500.csv")
write.csv(resultsEICtilde, "reruntilde500.csv")
write.csv(resultsEICPn, "rerunpn500.csv")
...
```{r}
data <- sim_data(n, F)
bias <- pmin(0.4+abs(0.1 + data$W1/2 + data$W/2 + data$A/3), 1.1) / 500^(0.25)
quantile(bias)
```
```{r}
...
n = 250
data <- sim_data(n, F)
biasg <- pmin(0.4+abs(0.1 + data$W1/2 + data$W/2 + data$A/3), 1) / n^(0.25)
biasQ <- pmin(0.4+abs(0.1 + data$W1/2 + data$W/2 + data$A/3), 0.6) / n^(0.25)
gbias <- bound(data$g1 + runif(n, -0.05, 0.05) + biasg, 0.05)
Qbias <- bound(data$EY + runif(n, -0.05, 0.05) + biasQ, 0.05)
...
est <- param_old$estimates(task, fold_number = "full")
est2 <-  est$psi
resultsOldTmle <- c(resultsOldTmle, est2)
write.csv(results, "biasrerun250.csv")
write.csv(resultsEICtilde, "biasreruntilde250.csv")
write.csv(resultsEICPn, "biasrerunpn250.csv")
write.csv(resultsOldTmle, "biasrerunOldTMLE250.csv")
