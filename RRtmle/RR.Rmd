...
factor_list <- list(LF_emp$new("W"), 
                    LF_fit$new("A", make_learner(Lrnr_mean)),
                     LF_fit$new("R", Lrnr_cv$new(make_learner(Lrnr_glm, max_depth = 2), full_fit = T), type = "mean")
)
lik <- Likelihood$new(factor_list)
lik <- lik$train(task)
lrnr_sieve <- make_learner(Lrnr_glmnet_relaxed, relax_lambda = "best")

lrnr_derived <-  Lrnr_universal$new(lrnr_sieve, through_weights = F, nbasis = 2, max_degree = 1)
...
sl <- make_super_learner(list(lrnr_xgboost1,lrnr_xgboost2, Lrnr_LRR_subst$new(), Lrnr_LRR_glm$new(method = "IPW")), task, likelihood )
sl_cv <- Lrnr_cv$new(list(lrnr_xgboost1,lrnr_xgboost2, Lrnr_LRR_subst$new(), Lrnr_LRR_glm$new(method = "IPW")))
...
trained$learner_fits$CV_Stack$cv_risk(losss)

v=data.table(pred = (trained$predict(rev)))
other = (trained$learner_fits$CV_Stack$predict(rev))
other
...
```{r}
cv_stack <- trained$learner_fits$CV_Stack
other = (cv_stack$predict_fold(rev, "validation"))
other
task <- cv_stack$training_task
task <- task$revere_fold_task("validation")
other = (cv_stack$predict_fold(task, "validation"))
other
```
...
cv_risk(trained$learner_fits$CV_Stack, loss)    
```
