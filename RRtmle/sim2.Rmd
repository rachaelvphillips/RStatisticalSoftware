...
print(n)
scores <- read.csv( paste0("sieve_scores_",n,".csv"))[,-1]
initscores <- read.csv(paste0("initial_scores_",n,".csv"))[,-1]
risks <- read.csv(paste0("risk_estimates",n,".csv"))[,-1]

risks1 <- as.vector(apply(risks,2,mean, na.rm = T))
risks1[1:3]
splitrisks <- split(risks1, (1:18) %%3)
splitrisks$`1` - splitrisks$`0`
splitrisks$`2` - splitrisks$`0`

  print("bias")
v1 <- as.vector(apply((risks[,3*(k-1) +1, drop = F] - splitrisks$`0`[k])^2,2,mean, na.rm = T))
v1 <- as.vector(apply((risks[,3*(k-1) + 1, drop = F]- splitrisks$`0`[k]),2,mean, na.rm = T))

v2 <- as.vector(apply((risks[,3*(k-1) + 2, drop = F] - splitrisks$`0`[k])^2,2,mean, na.rm = T))
v2 <- as.vector(apply((risks[,3*(k-1) + 2, drop = F]- splitrisks$`0`[k]),2,mean, na.rm = T))
print(sqrt(n)*c(v1,v2))

print("var")

v1 <- as.vector(apply((risks[,3*(k-1) +1, drop = F] - splitrisks$`0`[k])^2,2,sd, na.rm = T))
v1 <- as.vector(apply((risks[,3*(k-1) + 1, drop = F]- splitrisks$`0`[k]),2,sd, na.rm = T))

v2 <- as.vector(apply((risks[,3*(k-1) + 2, drop = F] - splitrisks$`0`[k])^2,2,sd, na.rm = T))
v2 <- as.vector(apply((risks[,3*(k-1) + 2, drop = F]- splitrisks$`0`[k]),2,sd, na.rm = T))
print(sqrt(n)*c(v1,v2))
}}
...
D <- D +
  ...
  node("g", distr = "rconst", const = plogis((W)) )+
...
library(sl3)
library(uuid)
npsem <- list(define_node("W", c("W"), time = 0, variable_type = variable_type("continuous")), define_node("A", "A", c("W"), time = 0),
...
mean_fun_R_bias <- function(task) {
  A <- task$data[["A"]]
  W <- task$data[["W"]]
  return(bound(plogis(W + 2*A - 1) + abs(.2 + W)/(n)^(0.25), 0.01))
}
mean_fun_A_bias <- function(task) {
  W <- task$data[["W"]]
  g1 <- (bound(plogis(W) + abs(.2 + W)/(n)^(0.25), 0.01))
  return(g1)
}
...
lrnr_fourier <- make_learner(Pipeline, Lrnr_cv$new(list(
  ...
  Lrnr_fourier$new(fourier_basis(4, 1), stratify_by = "A", mult_by = "g"),
   Lrnr_fourier$new(fourier_basis(5, 1), stratify_by = "A", mult_by = "g"),
     Lrnr_fourier$new(fourier_basis(6, 1), stratify_by = "A", mult_by = "g"),
     Lrnr_fourier$new(fourier_basis(7, 1), stratify_by = "A", mult_by = "g"),
...
write.csv(results_sieve, paste0("sieve_scores_",n,".csv"))
write.csv(results_xg, paste0("initial_scores_",n,".csv"))
write.csv(risk_estimates, paste0("risk_estimates",n,".csv"))
})
}
```
