...
for(k in 1:5) {
  ...
  for(n in c(1000,2500,5000,10000)) {
    key = paste0(k, "%", n)
print(n)
scores <- read.csv( paste0("GG_sieve_scores_",n,".csv"))[,-1]
initscores <- read.csv(paste0("GG_initial_scores_",n,".csv"))[,-1]
risks <- read.csv(paste0("GG_risk_estimates",n,".csv"))[,-1]
risks10000 <- read.csv(paste0("GG_risk_estimates",10000,".csv"))[,-1]
print("scores")
...
risk_estimates <- data.frame(matrix(NA, nrow = 1000, ncol = 5*4))
...
lrnr_fourier <- make_learner(Pipeline, Lrnr_cv$new(list(
  Lrnr_fourier$new(fourier_basis(2, 1),  mult_by = c("Qg1", "Qg0")),
    Lrnr_fourier$new(fourier_basis(2, 1),  mult_by = c("Qg1", "Qg0"))
   #Lrnr_fourier$new(fourier_basis(2, 1), stratify_by = "A", mult_by = "g")
), full_fit = T), Lrnr_cv_selector$new(loss_loglik_binomial))
} else if(n==2500){
lrnr_fourier <- make_learner(Pipeline, Lrnr_cv$new(list(
       Lrnr_fourier$new(fourier_basis(3, 1), mult_by = c("Qg1", "Qg0")),
    Lrnr_fourier$new(fourier_basis(4, 1), mult_by = c("Qg1", "Qg0"))
   #Lrnr_fourier$new(fourier_basis(2, 1), stratify_by = "A", mult_by = "g")
), full_fit = T), Lrnr_cv_selector$new(loss_loglik_binomial))
} else if(n==5000){
lrnr_fourier <- make_learner(Pipeline, Lrnr_cv$new(list(
...
lrnr_fourier <- make_learner(Pipeline, Lrnr_cv$new(list(
    Lrnr_fourier$new(fourier_basis(8, 1), mult_by = c("Qg1", "Qg0")),
  Lrnr_fourier$new(fourier_basis(7, 1),  mult_by = c("Qg1", "Qg0"))
   #Lrnr_fourier$new(fourier_basis(2, 1), stratify_by = "A", mult_by = "g")
), full_fit = T), Lrnr_cv_selector$new(loss_loglik_binomial))
}  else if(n==15000){
lrnr_fourier <- Lrnr_fourier$new(fourier_basis(10, 1), mult_by = c("Qg1", "Qg0"))
}
...
risk_estimates[i,] <- risks 
 ...
 write.csv(results_sieve, paste0("GG_sieve_scores_",n,".csv"))
 write.csv(results_xg, paste0("GG_initial_scores_",n,".csv"))
 write.csv(risk_estimates, paste0("GG_risk_estimates",n,".csv"))
})
}
```
