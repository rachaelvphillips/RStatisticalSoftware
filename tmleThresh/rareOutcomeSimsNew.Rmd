...
D <- D +
 ...
 node("Sf", distr = "rgamma",  shape = 3,rate=  13 ) +
 node("S", distr = "rconst",  const = min(Sf, 0.6) ) +
     node("g", distr = "rconst", const = dgamma(S,  shape = 5,rate=  8 + 3*W1 + W2 + 5*W3 )) +
  node("PY", distr = "rconst", const =  plogis(-4.7+
    0.7 * (
      0.7*(0.5-1.5*S + 0.25*(W1 + W2 + W3) + sin(3*W1) + sin(3*W2)+ log(1+W3) + 2*W1*sin(3*W2) + 2*W2*sin(3*W1) + W3*sin(3*W1) + W3 * ((S)-0.4) + W3*cos(3*W1))))) +
  node("Y", distr = "rbinom", size =1, prob = PY)
...
D <- D +
  ...
  node("W2", distr = "rconst", const = 4*round(W2full/4,1))+
  node("W3", distr = "rconst", const = round(0.2*min(W3full,3),1)/0.2)+
 node("Sf", distr = "rgamma",  shape = 3,rate=  15+ 3*W1 + W2 + 3*W3 ) +
 node("S", distr = "rconst",  const = min(Sf, 0.55) ) +
     node("g", distr = "rconst", const = dgamma(S,  shape = 5,rate=  8 + 3*W1 + W2 + 5*W3 )) +
  node("PY", distr = "rconst", const =  plogis(-4.7+
    0.7 * (
      0.7*(0.5-1.5*S + W1+W2+ W3 + 2*W1*sin(3*W2) + 2*W2*sin(3*W1) + W3*sin(3*W1) + W3 * ((S)-0.4) + W3*cos(3*W1))))) +
  node("Y", distr = "rbinom", size =1, prob = PY)
setD <- set.DAG(D, vecfun = "dgamma")
data <- sim(setD, n = 5)
...
print(truth)
print(ci)
write.csv(CI_SL, paste0("CI_SL_", n, ".csv"))
write.csv(covers_SL, paste0("covers_SL_", n, ".csv"))
write.csv(CI_glmnet, paste0("CI_glmnet_", n, ".csv"))
write.csv(covers_glmnet, paste0("covers_glmnet_", n, ".csv"))
write.csv(CI_donovan, paste0("CI_donovan_", n, ".csv"))
write.csv(covers_donovan, paste0("covers_donovan_", n, ".csv"))
#mean(data_full$Y * (data_full$S >= thresh)) / mean(data_full$S >= thresh)
#mean(data$wt * data$Y * (data$S >= thresh)) / mean(data$wt * (data$S >= thresh))
}
```
